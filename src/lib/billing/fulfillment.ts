...
export async function fulfillPaidOrder(order: Order): Promise<void> {
  ...
  // Renewals and plan changes are generated by Polar, not by a checkout this
  // app created, so they carry no checkout ID and none of our metadata. They
  // are trusted on the strength of the verified signature, the external
  // customer ID we set at subscription creation, and a known product. Every
  // other order must match the checkout intent that started it.
  const isRecurringCharge =
    order.billingReason === 'subscription_cycle' || order.billingReason === 'subscription_update'
  ...
  if (isRecurringCharge) {
    if (!order.subscription || !planForProductKey(product.productKey)) {
      throw new UntrustedBillingEventError('Recurring charge without a plan subscription')
    }
  } else {
    intentId = metadataString(order.metadata.checkout_intent_id)
    const metadataProductKey = metadataString(order.metadata.product_key)
    if (!order.checkoutId || !intentId || !metadataProductKey) {
      throw new UntrustedBillingEventError('Missing trusted checkout metadata')
    }

    const [intent] = await db.select().from(checkoutIntents)
      .where(eq(checkoutIntents.id, intentId)).limit(1)
  ...
  } else if (planForProductKey(product.productKey)) {
    if (!order.subscription) throw new UntrustedBillingEventError('Subscription product has no subscription')

    // Access and the monthly allowance go through the same period-keyed path
    // the subscription.* webhooks use. Polar sends both order.paid and
    // subscription.active for a new subscriber, and both again on renewal, so
    // a single shared key per billing period is what prevents a double grant.
    const state: SubscriptionState = {
      providerSubscriptionId: order.subscription.id,
      userId: user.id,
      providerProductId: order.subscription.productId,
      status: String(order.subscription.status),
...
export async function grantSubscriptionPeriodCredits(
  ...
  // Keyed to the calendar month rather than the billing period, so a monthly
  // plan gets one grant per renewal and an annual plan still gets one per
  // month instead of a single grant for the whole year. The first grant uses
  // the period start, so it lands in the month the subscription began.
  const anchor = now < state.currentPeriodStart ? state.currentPeriodStart : now
  const month = anchor.toISOString().slice(0, 7)

  // A monthly allowance never outlives roughly a month, even on an annual
  // plan whose billing period runs a year.
  const expiresAt = new Date(Math.min(
    state.currentPeriodEnd.getTime(),
    anchor.getTime() + 35 * 24 * 60 * 60 * 1000,
  ))
  ...
  await grantCredits({
    ...
    sourceType: 'subscription',
    sourceId: state.providerSubscriptionId,
    idempotencyKey: `subscription:${state.providerSubscriptionId}:monthly:${month}`,
    expiresAt,
  })
}
